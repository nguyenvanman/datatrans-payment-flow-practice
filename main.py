...
app = FastAPI(title="Datatrans Payment Demo")
...
async def init_payment(payload: InitPaymentRequest):
    ...
    try:
        ...
        if response.status_code != 201:
            raise HTTPException(
                status_code=502, detail=f"Datatrans error: {response.text}"
            )

        data = response.json()
        transaction_id = data.get("transactionId")
        payment_page_url = f"{DATATRANS_BASE_URL}/v1/start/{transaction_id}"
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                new_status = status_map.get(
                    remote.get("status", "").lower(), txn.status
                )
